...
function generate1CAgentScript(cfg) {
    const lines = [
        ...
        'echo   Server: ' + cfg.server + ':' + cfg.port,
        'echo   Base: ' + cfg.baseName,
        'echo ======================================',
        'echo.',
        '',
        ...
        "Write-Host ('Server: ' + $server + ':' + $port + ' Base: ' + $base) -ForegroundColor Gray",
        'try {',
        "    $connector = New-Object -ComObject V83.COMConnector",
        "    $connStr = 'Srvr=\"' + $server + '\";Ref=\"' + $base + '\";Usr=\"' + $login + '\";Pwd=\"' + $password + '\"'",
        '    $conn = $connector.Connect($connStr)',
        "    Write-Host 'Connected to 1C successfully!' -ForegroundColor Green",
        '',
        '    # --- Query products with prices ---',
        "    $query = $conn.NewObject('Query')",
        '',
        '    # Try Russian query first (most common for IgotoShop / 1C RU)',
        '    try {',
        "        $query.Text = @'",
        'ВЫБРАТЬ',
        '    Ном.Код КАК Barcode,',
        ...
        '    ЕСТЬNULL(Ном.ЕдиницаИзмерения.Наименование, \"шт\") КАК Unit,',
        '    ЕСТЬNULL(Ном.Родитель.Наименование, \"Прочее\") КАК Category',
        'ИЗ',
        ...
        "        Write-Host 'Query OK (Russian)' -ForegroundColor Green",
        '    } catch {',
        "        Write-Host ('Russian query failed: ' + $_.Exception.Message) -ForegroundColor Yellow",
        "        Write-Host 'Trying without price register...' -ForegroundColor Yellow",
        '        try {',
        "            $query.Text = @'",
        'ВЫБРАТЬ',
        '    Ном.Код КАК Barcode,',
        ...
        "'@",
        '            $result = $query.Execute().Unload()',
        "            Write-Host 'Query OK (without prices)' -ForegroundColor Yellow",
        '        } catch {',
        "            Write-Host 'Trying English query...' -ForegroundColor Yellow",
        "            $query.Text = 'SELECT Code AS Barcode, Description AS Name FROM Catalog.Nomenclature WHERE DeletionMark = FALSE AND IsFolder = FALSE'",
        '            $result = $query.Execute().Unload()',
        "            Write-Host 'Query OK (English)' -ForegroundColor Green",
        '        }',
        '    }',
        '',
        '    $items = @()',
        ...
        '        $row = $result.Get($i)',
        '        $item = @{ barcode = [string]$row.Barcode; name = [string]$row.Name }',
        '        try { $item.price = [double]$row.Price } catch { $item.price = 0 }',
        '        try { $item.unit = [string]$row.Unit } catch { $item.unit = \"шт\" }',
        '        try { $item.category = [string]$row.Category } catch { $item.category = \"Прочее\" }',
        '        $items += $item',
        '    }',
        ...
        '    # Show sample',
        '    if ($items.Count -gt 0) {',
        "        Write-Host '--- Sample (first 3): ---' -ForegroundColor Gray",
        '        $items | Select-Object -First 3 | ForEach-Object {',
        "            Write-Host ('  ' + $_.barcode + ' | ' + $_.name + ' | ' + $_.price + ' sum') -ForegroundColor Gray",
        '        }',
        '    }',
        '',
        '    $conn = $null',
        '',
        '    # --- Send to cloud ---',
        "    Write-Host 'Sending to cloud...' -ForegroundColor Cyan",
        '    $body = @{ apiKey = $apiKey; storeId = $storeId; items = $items } | ConvertTo-Json -Depth 5 -Compress',
        "    $resp = Invoke-RestMethod -Uri $cloudUrl -Method POST -Body ([System.Text.Encoding]::UTF8.GetBytes($body)) -ContentType 'application/json; charset=utf-8'",
        "    Write-Host ('Synced! Added: ' + $resp.added + ', Updated: ' + $resp.updated + ', Total: ' + $resp.total) -ForegroundColor Green",
        '} catch {',
